            ...
            if (args[0].toLowerCase() == "view") {
               if(args[1]==null){
                   ...
                   switch (args[1].toLowerCase()) {
                        case "open":
                            {
                                connection.query(`SELECT * FROM ${process.env.mysql_bug_reports_table} WHERE status = 'Open'ORDER BY made_at ASC LIMIT 10`,function(err,results){
                        ...
                        case "pending":
                            {
                                connection.query(`SELECT * FROM ${process.env.mysql_bug_reports_table} WHERE status = 'Pending'ORDER BY made_at ASC LIMIT 10`,function(err,results){
                                    let embed = new Discord.MessageEmbed()
                                    if(results.length == 0){
                        ...
                        case "all":
                            {
                                connection.query(`SELECT * FROM ${process.env.mysql_bug_reports_table} WHERE status NOT LIKE 'Closed' ORDER BY made_at ASC LIMIT 10`,function(err,results){
                                    let embed = new Discord.MessageEmbed()
                                    if(results.length == 0){
                        ...
                        case "closed":
                            {
                                connection.query(`SELECT * FROM ${process.env.mysql_bug_reports_table} WHERE status = 'Closed'ORDER BY made_at ASC LIMIT 10`,function(err,results){
                                    let embed = new Discord.MessageEmbed()
                                    if(results.length == 0){
                        ...
                        case "id":
                            {
                                connection.query(`SELECT * FROM ${process.env.mysql_bug_reports_table} WHERE id = ${args[2]} ORDER BY made_at ASC LIMIT 10`,function(err,results){
                                    let embed = new Discord.MessageEmbed()
                                ...
                                connection.query(`UPDATE ${process.env.mysql_bug_reports_table} SET status = "Open" WHERE id = ${args[2]}`,function(err,results){
                                })
                            }
                        break;
                ...
                connection.query(`UPDATE ${process.env.mysql_bug_reports_table} SET status = "Closed" WHERE id = ${args[1]}`);
                message.channel.send(new Discord.MessageEmbed().setTitle("Closed Case id " + args[1]))
            }else if(args[0].toLowerCase() == "delete"){
                connection.query(`DELETE FROM ${process.env.mysql_bug_reports_table} WHERE id = ${args[1]}`);
